...
// ring r2, point v2
module.exports = function (polygon, r1, v1, r2, v2) {
  ...
  if (r1 === 0 && r2 === 0) {
    ...
    let polygons = p1.length > p2.length ? [[p1], [p2]] : [[p2], [p1]]
    for (let i = 1; i < polygon.length; i++) {
      const ring = polygon[i]
      if (inside(ring[0], polygons[0][0])) {
        polygons[0].push(ring)
      } else {
        polygons[1].push(ring)
      }
    }
    return polygons
  } else {
